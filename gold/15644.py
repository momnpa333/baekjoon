...
dq=deque([])
path=""
dq.append([0,redR,redC,blueR,blueC,board,path])
check=set()

while dq:
    for _ in range(len(dq)):
        depth,redR,redC,blueR,blueC,b,path=dq.popleft()
        check.add((redR,redC,blueR,blueC))
        # print(depth)
        for idx,dir in enumerate(((0,1),(1,0),(0,-1),(-1,0))):
            addr,addc=dir
            if (addr,addc)==(0,1):
                if redC>blueC:
            ...
            if (tredR,tredC,tblueR,tblueC) not in check:
                dq.append([depth+1,tredR,tredC,tblueR,tblueC,boardcopy,path+DIR[idx]])
        if depth==10:
            print(-1)
            exit(0)
print(-1)